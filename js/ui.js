...
function renderRiscoFilter() {
  ...
  var riscoItemsMapped = appState.riscoOptions.map(function(r) {
    return ({
      value: r.toLowerCase(),
...
function renderActiveFilterTags() {
  ...
  if (appState.filters.isAcoesOnly) {
    tags.push({
      key: 'acoes',
      label: 'Apenas Fundos de Ações',
      onClear: function() {
        if (ui.acoesFilterToggle) {
          ui.acoesFilterToggle.checked = false;
          ui.acoesFilterToggle.dispatchEvent(new Event('change'));
        }
      }
    });
  }
...
function renderDesktopView(pageData) {
  ...
  pageData.forEach(function(item, index) {
    ...
    var isNet = appState.taxBracket !== 'bruta' && !item.isento;
    ...
    tdNome.title = escapeHtml(item.fullName);
    tdNome.innerHTML = buildNomeComTag(item);
    ...
    rentabilidadeDiv.className = 'text-sm flex items-center ' + getPerformanceColor(rawValue);
    rentabilidadeDiv.title = escapeHtml(title || titleContext || '');
    rentabilidadeDiv.innerHTML = displayNode + (isNet ? '<span class="text-red-600 ml-1">*</span>' : '');
    ...
    tdResgate.title = escapeHtml(item.resgateOriginal || '');
    tdResgate.textContent = item.resgateDisplay || '-';
    ...
    riscoSpan.className = 'px-2 py-1 inline-flex text-xs leading-4 font-semibold rounded-full ' + getRiscoClasses(item.risco);
    riscoSpan.textContent = (appState.isMobile ? getRiscoAbreviado(item.risco) : (item.risco || 'N/A'));
    ...
    detailCell.innerHTML = renderDetailPanel(item);
...
function renderMobileView(pageData) {
  var fragment = document.createDocumentFragment();
  pageData.forEach(function(item) {
    var cardElement = renderMobileCard(item);
...
function renderDetailPanel(item) {
  ...
  var rentabilidadeItem = function(periodKey, label) {
    ...
    return detailItem(ICONS.Calendar, label,
      ...
      '>' +
      displayNode + (isNet ? '<span class="text-red-600 ml-1">*</span>' : '') +
      '</div>'
    );
  };
  ...
  return (
    ...
    detailItem(ICONS.Download, "Lâmina do Fundo",
      (function() {
        ...
        if (isValidHttpUrl(item.lamina_link)) {
          return '<a' +
            ' href="' + escapeHtml(item.lamina_link) + '"' +
            ' target="_blank"' +
        ...
        return '<p class="text-base font-medium text-red-500" title="URL malformada: ' + escapeHtml(item.lamina_link) + '">Link inválido</p>';
      })()
    ) +
    '</div>' +
    '</div>'
...
function renderMobileCard(item) {
  ...
  nameH4.title = escapeHtml(item.fullName);
  nameH4.innerHTML = buildNomeComTag(item);
  ...
  var rentabilidadeContent = '<div class="text-xl font-bold ' + getPerformanceColor(rawValue) + ' flex items-center" title="' + escapeHtml(title || titleContext || '') + '">' +
    displayNode + (isNet ? '<span class="text-red-600 ml-0.5">*</span>' : '') + '</div>';
  bodyGridDiv.appendChild(createGridItem('Rentab. (' + periodLabels[displayPeriodKey] + ')', rentabilidadeContent));

  var riscoContent = '<div class="mt-1"><span class="px-2 py-1 inline-flex text-xs leading-4 font-semibold rounded-full ' + riscoClasses + '">' +
    (item.risco || 'N/A') + '</span></div>';
  bodyGridDiv.appendChild(createGridItem('Risco', riscoContent));

  var resgateContent = '<div class="text-base font-medium text-gray-900" title="' + escapeHtml(item.resgateOriginal || '') + '">' +
    (item.resgateDisplay || '-') + '</div>';
  ...
  detailDiv.innerHTML = renderDetailPanel(item);
