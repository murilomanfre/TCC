...
function processLoadedData(json) {
  try {
    ...
    processRiscoOptions(json.dados);
    renderRiscoFilter();
  } catch (error) {
...
function handleFileLoad(event) {
  ...
  reader.onload = function(e) {
    ...
    if (!content || content.trim() === '') {
      showModal("Erro ao carregar arquivo", "O arquivo selecionado está vazio."); // v3.49.0
      appState.isLoading = false;
      runDataPipeline();
      return;
    }
    ...
    try {
      ...
      showModal("Erro ao carregar arquivo", "O formato do arquivo é inválido. Certifique-se de que seja um JSON válido ou um arquivo de texto contendo JSON codificado em Base64. Detalhe: " + error.message); // v3.49.0
      appState.isLoading = false;
      runDataPipeline();
    }
  };
